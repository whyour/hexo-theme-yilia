	            ...
	            channel.port1.onmessage = function (e) {
	                ...
	                if (e.data.type === 'applyNotify') {
	                    window.Notification.requestPermission().then(function (grant) {
	                        ...
	                        if (navigator.serviceWorker.controller) navigator.serviceWorker.controller.postMessage({ type: 'notify', info: e.data.info }, [channel.port2]);
	                    });
	                }
	            };

	            if (navigator.serviceWorker.controller) navigator.serviceWorker.controller.postMessage('hello', [channel.port2]);
	        });
